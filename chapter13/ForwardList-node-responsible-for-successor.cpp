// also available live: https://wandbox.org/permlink/5Tn7LxLc5IP4WgkN
...
public:
   ...
   iterator insert_after(iterator pos, const_reference value) {
      ...
      p->next.reset(pos.cur->next.get()); // <--
      pos.cur->next.release(); // <--
      pos.cur->next.reset(p.get()); // <--
      p.release(); // <--
      ++nelems;
      return { pos.cur->next.get() }; // <--
   }
