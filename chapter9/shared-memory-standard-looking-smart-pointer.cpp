...
template <class T>
   ...
   public:
      template <class ... Args>
         shared_mem_ptr(shared_mem_mgr &mgr, Args &&... args)
            : mgr{ mgr }, p{ new (mgr) T(std::forward<Args>(args)...) } {
         }
      shared_mem_ptr(const shared_mem_ptr&) = delete;
      shared_mem_ptr& operator=(const shared_mem_ptr&) = delete;
      ~shared_mem_ptr() {
         p->~T();
         operator delete(p, sizeof(T), mgr);
      }
      T* operator->() { return p; }
...
int main() {
   ...
   auto p_data = shared_mem_ptr<data>{ mgr, false };
